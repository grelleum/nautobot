...
def _modify_default_table_columns(table_extension, app_name):
    ...
    table = get_table_for_model(table_extension.model)
    add_to_default_columns = [
        name for name in table_extension.add_to_default_columns if name in table.base_columns
    ]

    if table_extension.add_to_default_columns:
        table.Meta.default_columns = (*table.Meta.default_columns, *add_to_default_columns)

    if table_extension.remove_from_default_columns:
        table.Meta.default_columns = tuple(
            column for column in table.Meta.default_columns
            if column not in table_extension.remove_from_default_columns
        )
